...
echo "\n"
echo "\nUsage: "
echo "\n  ./conda_fresh_install.sh [DIR] [VENV] [GIT] [PROTOBUF] [DATA] [JAVA]"
echo "\n  NEW_DIR:  [XXXX/nodir]"
echo "\n  VENV:     [venv/novenv]"
echo "\n  GIT:      [git/nogit]"
echo "\n  BRANCH:   [XXXX/main]"
...
then
    ...
    if [[ $NEW_CONDA_ENV =~ ^venv$ ]]
    then
        echo "Setting up new venv env..."
        python -m venv $NEW_DIR-env
    fi

    # Activate the env
    echo "Activating venv env..."
    $NEW_DIR-env/bin/activate
